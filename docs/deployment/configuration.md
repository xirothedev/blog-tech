...
The application uses a sophisticated multi-stage Docker build process with Yarn Berry:
...
- **Immutable Builds**: Yarn Berry for deterministic builds
...
[build]
  publish = "out"
  command = "EXPORT=true UNOPTIMIZED=1 yarn build"
    ...
    steps:
      ...
      - uses: actions/setup-node@v4
        with:
          node-version: '24'
      - run: yarn install
      - run: EXPORT=true UNOPTIMIZED=1 yarn build
...
**Build Optimization**:
- **Yarn Berry**: Modern package manager with immutable installs
...
# Static Export
EXPORT=true yarn build      # Generate static files
...
# Static Build
EXPORT=true NODE_ENV=production yarn build
...
yarn start --experimental-debug-mode
```
